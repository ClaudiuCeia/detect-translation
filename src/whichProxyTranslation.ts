...
const PROXIES_HOSTNAMES: { [key in Services]?: Array<HostnamePattern> } = {
  ...
  [Services.MICROSOFT]: ["www.translatoruser-int.com", "www.translatetheweb.com", /^(ssl|www)\.microsofttranslator\.com$/],
...
const whichProxyTranslation = (identified: LangTranslatorInfo): LangTranslatorInfo => {
  const { hostname } = location;

  const [service] = (
    Object.entries(PROXIES_HOSTNAMES).find(([, hostnames]) =>
